...
def bucket_sort(lista):
    ...
    for i in range(len(lista)):
        j = int (lista[i] / size)
        if j != len (lista):
            arr[j].append(lista[i])
        else:
            arr[len(lista) - 1].append(lista[i]) 
...
def random_select(a,l,r,i):
    ...
    #val = a[p]
    q = partition_r(a,l,r,p)
    k = q - l + 1
    if(i == k): 
        return a[q]
    elif (i < k): 
        return random_select(a,l,q-1,i)
    else: 
...
a=["vaca","perro","alfa","lobo","zorro","media","corazon","leticia","uwU"]
b=[4,2,57,26,26,4534,624,346,2,768,95,234,22]
s = random_select(b,0,len(b)-1,1)
print(s)
ta = len(a)
tb = len(b)
